...
input=new File("MNT.txt");fr=new FileReader(input);br=new
BufferedReader(fr);
while((line=br.readLine())!=null)
{
String[] a=line.split("[ ]");
...
input=new File("calls.txt");fr=new FileReader(input);br=new
BufferedReader(fr);
while((line=br.readLine())!=null) {
if(line.substring(0,4).equals("CALL"))
{
//output.write("NEW MACRO\n");
int index;
...
String []a=line.split("[, ]");
    ...
    if(!a[i].substring(1+a[i].indexOf('=')).equals(" "))
     kpdtab.put(a[i].substring(0,a[i].indexOf('=')),
a[i].substring(1+a[i].indexOf('=')));}

     }
   //  System.out.println(name.get(index));
    // for (String key : kpdtab.keySet())System.out.println(key+"
"+kpdtab.get(key));
     int cnt=0;
     for (String key : kpdtab.keySet()) { cnt++;
    if(cnt<kpdt.get(index)-1) {
    continue;}
    if(cnt>kpdt.get(index)+kp.get(index))break;
    if(!kpdtab.get(key).equals(" "))
         aptab.add(kpdtab.get(key));
     }
     for(int i=0;i<aptab.size();i++)System.out.println(aptab.get(i));
